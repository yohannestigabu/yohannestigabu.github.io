# 👋 Hi, I'm Yohannes Tigabu  
### Machine Learning & Data Science Portfolio  

Welcome! I'm a data-driven learner passionate about turning raw information into insights and intelligent systems.  
I build machine learning projects that focus on real-world applications — safety, prediction, and decision support.

---

## 🚗 KSI Collision Severity Predictor  
**Goal:** Predict whether a Toronto traffic collision will result in a **Fatal** or **Non-Fatal Injury** using open data.  
**Dataset:** Toronto Police Service Killed or Seriously Injured (KSI) Open Data  
**Tech Stack:** Python • Pandas • Scikit-Learn • Streamlit  
**Result:** 76% model accuracy with clear interpretability and an interactive app.

- 🔗 [GitHub Repository](https://github.com/yohannestigabu/ksi-collision-predictor)  
- 🌐 [Live App on Streamlit](https://yohannestigabu-ksi-collision-predictor.streamlit.app)

---

## 🧠 Projects in Progress  
- 🩺 Diabetes Prediction using Deep Learning  
- 🗺️ Toronto Crime Clustering with K-Means  

---

## 🧰 Skills  
**Languages:** Python, SQL  
**ML Tools:** Scikit-Learn, TensorFlow, Keras, Pandas, NumPy  
**Visualization:** Matplotlib, Seaborn, Streamlit, Plotly  
**Version Control:** Git, GitHub  
**Data Engineering (in progress):** Databricks, PySpark  

---

## 💼 About Me  
I’m a hands-on learner who enjoys building end-to-end solutions — from raw data to deployed apps.  
My focus areas include predictive modeling, data storytelling, and practical AI applications.

---

## 📫 Contact  
- 📧 **Email:** yohannestigabu1@gmail.com  
- 💼 [LinkedIn]([https://linkedin.com/in/your-linkedin](https://www.linkedin.com/in/yohannes-tigabu-fetle-938aba108/))  
- 💻 [GitHub](https://github.com/yohannestigabu)
